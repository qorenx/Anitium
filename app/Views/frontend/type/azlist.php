...
// Extract the last non-empty segment of the URL path as the page name
$url = '';
for ($i = count($page_url) - 1; $i >= 0; $i--) {
    if (!empty($page_url[$i])) {
        $url = $page_url[$i];
        break;
    }
...
<div class="pagination">
    <nav>
        <ul class="ulclear az-list">
            <?php 
                $api_url = "https://example.com/api"; // Set API base URL
                $AZPage = false;
                
                // Fetch data from API
                try {
                    $endpoint = "$api_url/anime-AZ-page?aph=$url&page=$page";
                    $response = file_get_contents($endpoint);

                    if ($response !== false) {
                        // Decode JSON response
                        $data = json_decode($response, true);
                        $AZPage = $data['pagination'];
                    }
                } catch (Exception $e) {
            ...
            ?>
                ...
                <?php echo view('innerpage/sidenav.php');?>
                <div class="clearfix"></div>
            </div>
        </div>
        <?php echo view('innerpage/footer.php');?>
